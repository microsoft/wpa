...
This output is useful to identify groups that are worth investigating (e.g. employees labelled as `"FunctionType = Sales"`). It also reveals that `FunctionType` and `Organization` are interesting attributes to split the data, as the have significant variation. In contrast, for `LevelDesignation` all groups appear to have broadly similar levels of collaboration (between 18 and 22 hours a week), with no group above others by a significant amount.  
