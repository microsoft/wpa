...
<B>
  <font size="+3">
    Resilience - Can teams respond to urgent needs without risking employee wellbeing?
  </font>
</B>
<br>
<br>
...
urg_sum_tb <-
  ...
  my_sq_data %>%
    group_by(!!sym(hrvar)) %>%
    summarise(across(IsUrgent, ~sum(., na.rm = TRUE))) %>%
...
urg_sum_tb %>%
  ...
  labs(
    ...
    subtitle = "Bubble size measures number of people in organization",
    x = "% of employees affected by urgent collaboration",
    y = "Number of urgent weeks",
...
scales::percent((afch_urg - afch_nonurg) / afch_nonurg) %>%
