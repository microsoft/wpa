...
col_checker <- function(x, col, replacement = NULL){
  ...
  if(col %in% names(x) & is.null(replacement)){
    ...
    # column exists, replacement not specified
    # ACTION: RETURN
    x

  } else if(!is.null(replacement)){
    
    if(replacement %in% names(x)){
      ...
      # column exists, replacement specified
      # ACTION: RENAME
    ...
    # column not exist, replacement specified
    # ACTION: DUMMY COLUMN WITH REPLACEMENT
    dplyr::mutate(x, !!sym(replacement) := NA) # create dummy column

  } else if(!(col %in% names(x)) & is.null(replacement)){

    # column not exist, replacement not specified
    # ACTION: DUMMY COLUMN WITH COLUMN
...
my_sq_data <-
  ...
  col_checker(col = "Intimate_1_on_1_meeting_hours") %>%
  col_checker(col = "Intimate_group_meeting_hours") %>%
  col_checker(col = "Instant_messages_sent") %>%
