...
<B>
  <font size="+3">
    Flexibility - Are employees able to switch off?
  </font>
</B>
<br>
<br>
...
flexibility1_w1_plot1 +
  labs(title = "Classification of activity patterns",
       subtitle = "Frequency of common archetypes")
...
KPI_2 <-  
  wp_list$table %>%
  filter(Personas =="6 Always on (13h+)") %>%
  pivot_longer(!Personas, names_to = "group", values_to = "alwaysOn") %>%
  filter(group %in% mingroup_str) %>%
  filter(alwaysOn == max(alwaysOn)) %>%
  select(group) %>%
  pull(1)
...
### <B>Which teams are most affected? </B> <br> Identify groups with highest prevalence the negative work patterns and  encourage them to disconnect and build regular breaks in their work day.

```{r}

#   flexibility1_w2_plot1 <- wp_list$plot_hrvar +
#   labs(title = "Distribution of Working Patterns",
#        subtitle = paste("By", camel_clean(hrvar))) + theme_wpa_basic() + 
#   theme(axis.title = element_blank())
# 
# flexibility1_w2_plot1 +
#   labs(title = "Working pattern distribution",
#        subtitle = "Frequency of employee work-weeks",
...
wp_list$data %>%
  mutate(Personas = case_when(Personas == "0 < 3 hours on" ~ "Low activity",
